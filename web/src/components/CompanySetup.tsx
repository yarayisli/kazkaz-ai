    ...
    <main className="mx-auto grid min-h-[calc(100vh-68px)] max-w-6xl place-items-center px-4 py-8 sm:py-12">
      <div className="grid w-full overflow-hidden rounded-3xl border border-white/10 bg-[#070b18] shadow-2xl shadow-black/20 lg:grid-cols-[0.72fr_1.28fr]">
        <section className="border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(124,58,237,.2),transparent_48%),#0b1020] p-7 sm:p-9 lg:border-b-0 lg:border-r">
          ...
          <p className="mt-7 text-xs font-black uppercase tracking-[0.2em] text-violet-300">Kontrollü başlangıç</p>
        ...
        <section className="p-6 sm:p-9 lg:p-11">
          ...
          <div className="mb-7 flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Adım {step + 1} / 4</p><h2 className="mt-2 text-xl font-bold text-white">{stepNames[step]}</h2></div><div className="flex gap-1.5" aria-label={`Adım ${step + 1} / 4`}>{[0, 1, 2, 3].map((index) => <span key={index} className={`h-2 rounded-full transition-all duration-300 ${index === step ? 'w-7 bg-violet-500' : index < step ? 'w-2 bg-emerald-500' : 'w-2 bg-white/10'}`} />)}</div></div>
          ...
          <div key={step} className="landing-reveal">
            ...
            {step === 2 && <div className="space-y-7">
              ...
              <div><div className="mb-3 flex items-end justify-between gap-3"><p className="text-xs font-bold text-slate-300">Şu anda elinizde olanlar</p><span className="text-xs text-slate-500">Birden çok seçin · bilmiyorsanız boş bırakın</span></div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{veriAlanlari.map((item) => <ChoiceButton key={item.value} selected={availableData.includes(item.value)} label={item.label} onClick={() => toggleData(item.value)} className={optionClass(availableData.includes(item.value))} />)}</div></div>
