    ...
    <div className="overview-panel space-y-5 pb-8">
      <section className="flex flex-col gap-3 rounded-xl border border-violet-200 bg-gradient-to-r from-[#eef2ff] to-[#f5f3ff] px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">{readyCount}/{readinessAreas.length} veri alanı analiz için kullanılabilir · sonuçlar yalnızca yüklenen girdilere dayanır.</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full border px-2.5 py-1 text-xs font-black uppercase tracking-[0.14em] ${isSampleData ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}>{isSampleData ? 'Örnek veri' : 'Şirket verisi'}</span>
            </div>
          ...
          return (
            <article key={kpi.label} className="panel-card panel-kpi-card relative overflow-hidden p-5 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(15,34,82,.09)]">
              ...
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-extrabold uppercase tracking-[0.13em] text-slate-500">{kpi.label}</p>
              ...
              <p className="mt-2 text-xs leading-4 text-slate-500">{kpi.note}</p>
            </article>
          );
        })}
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.45fr_.85fr]">
        <article className="panel-card p-5">
          ...
          <p className="mt-3 flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50 p-3 text-xs leading-5 text-blue-800"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0 text-blue-600" />Bu grafik büyüme trendi değildir. Yalnızca {data.period} döneminde girilen gelir tablosu kalemlerini karşılaştırır.</p>
        </article>
            ...
            {decisionQueue.slice(0, 3).map((item) => (
              <button key={item.title} type="button" onClick={() => onNavigateTab(item.tab)} className="panel-ai-finding group block w-full py-4 text-left">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2 py-1 text-xs font-extrabold uppercase tracking-wide ${item.tone === 'emerald' ? 'bg-emerald-50 text-emerald-700' : item.tone === 'blue' ? 'bg-blue-50 text-blue-700' : item.tone === 'violet' ? 'bg-violet-50 text-violet-700' : 'bg-amber-50 text-amber-700'}`}>{item.status}</span>
                </div>
                <p className="mt-2 text-xs leading-5 text-slate-500">{item.description}</p>
                <span className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-violet-700 opacity-0 transition group-hover:opacity-100">Detayı aç <ArrowRight className="h-3 w-3" /></span>
              </button>
            ))}
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
        <article className="panel-card p-5">
          <div className="flex items-start justify-between gap-5">
            ...
            <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full p-2" style={{ background: `conic-gradient(#7c3aed ${readinessScore}%, #eef0f4 0)` }}><div className="grid h-full w-full place-items-center rounded-full bg-white text-center"><div><span className="text-xl font-black text-[#0a1628]">%{readinessScore}</span><span className="block text-xs uppercase tracking-wider text-slate-400">hazır</span></div></div></div>
          </div>
          <div className="mt-5 grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
            {readinessAreas.map((area) => <div key={area.name} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5">{area.ready ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> : <CircleAlert className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" />}<div className="min-w-0"><p className="text-xs font-bold text-slate-700">{area.name}</p><p className="mt-0.5 text-xs leading-4 text-slate-500">{area.detail}</p></div></div>)}
          </div>
        </article>
        ...
        <article className="panel-card p-5">
          ...
          <div className={`mt-5 rounded-xl border p-3 ${calculatedNetProfit == null ? 'border-amber-200 bg-amber-50' : reconciles ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}><div className="flex items-center gap-2 text-xs font-bold text-slate-800">{calculatedNetProfit != null && reconciles ? <CheckCircle2 className="h-4 w-4 text-emerald-600" /> : <CircleAlert className="h-4 w-4 text-amber-500" />}{calculatedNetProfit == null ? 'Köprü kurulamadı' : reconciles ? 'Net kâr mutabık' : 'Mutabakat farkı var'}</div><p className="mt-1.5 text-xs leading-4 text-slate-600">{calculatedNetProfit == null ? 'Faiz, vergi ve amortisman ayrımı doğrulanmalıdır.' : `Fark: ${formatTRY(reconciliationDifference ?? 0)} · tolerans ${formatTRY(reconciliationTolerance)}`}</p></div>
        </article>
      </section>
      ...
      <section className="panel-card p-5">
        ...
        {!audit?.metrik_kaydi ? <div className="mt-5 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4"><CircleAlert className="mt-0.5 h-5 w-5 shrink-0 text-amber-500" /><div><p className="text-sm font-bold text-amber-900">Formül kaydı henüz oluşmadı</p><p className="mt-1 text-xs leading-5 text-amber-800/70">Finansal dosyayı yeniden aktarın veya alanları kaydedin; motor hesaplanabilen metrikler için denetim kaydı oluşturacaktır.</p></div></div> : <><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{metricRecords.map(([name, metric]) => { const info = kurumsalEtiketler[name]; const calculated = metric.durum === 'hesaplandi' && metric.deger != null; return <article key={name} className="rounded-xl border border-slate-200 bg-slate-50/60 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-bold text-slate-800">{info.etiket}</h3><p className="mt-1 text-xs leading-4 text-slate-500">{info.aciklama}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-xs font-bold uppercase ${calculated ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{calculated ? `${metric.guven} güven` : 'Veri gerekli'}</span></div><p className={`mt-5 text-2xl font-black ${calculated ? 'text-[#0a1628]' : 'text-slate-400'}`}>{calculated ? corporateValue(metric.deger as number, metric.birim) : 'Hesaplanmadı'}</p><div className="mt-4 border-t border-slate-200 pt-3 text-xs leading-4 text-slate-500"><p className="font-mono text-slate-600">{metric.formula_id} · {metric.formul_surumu}</p><p className="mt-1">{metric.metodoloji_notu}</p></div></article>; })}</div>{missingCorporateFields.length > 0 && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-xs font-bold text-amber-900">Kurumsal metrikler için eksik girdiler</p><div className="mt-3 flex flex-wrap gap-2">{missingCorporateFields.map((field) => <span key={field} className="rounded-full border border-amber-200 bg-white px-2.5 py-1 text-xs text-amber-800">{alanEtiketleri[field] ?? field.replaceAll('_', ' ')}</span>)}</div></div>}</>}
      </section>

      <section className="grid gap-5 lg:grid-cols-2">
        <article className="panel-card p-5"><div className="flex items-center gap-3"><span className="rounded-lg bg-amber-50 p-2 text-amber-600"><FileCheck2 className="h-5 w-5" /></span><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-700">Eksik veri</p><h2 className="text-lg font-bold text-[#0a1628]">Analizi güçlendirecek alanlar</h2></div></div>{missingInputs.length ? <div className="mt-4 flex flex-wrap gap-2">{missingInputs.map((field) => <span key={field} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs text-slate-600">{field}</span>)}</div> : <p className="mt-4 text-sm text-emerald-700">Temel analiz girdileri tamamlandı.</p>}<button type="button" onClick={() => onNavigateTab('data-entry')} className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-violet-700">Excel veya form ile tamamla <ArrowRight className="h-3.5 w-3.5" /></button></article>
        <article className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 to-indigo-50 p-5 shadow-sm"><div className="flex items-center gap-3"><span className="rounded-lg bg-violet-100 p-2 text-violet-700"><Bot className="h-5 w-5" /></span><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-700">AI CFO çalışma ilkesi</p><h2 className="text-lg font-bold text-[#0a1628]">Önce metrik, sonra yorum</h2></div></div><p className="mt-4 text-sm leading-6 text-slate-600">AI önerisi; dayandığı metriği, veri kapsamını ve güven seviyesini gösterir. Eksik veride kesin aksiyon üretmez; ödeme, yatırım ve borç kararlarında insan onayı ister.</p><button type="button" onClick={() => onNavigateTab('cfo-agent')} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#0f2252] px-4 py-2.5 text-xs font-black text-white transition hover:bg-[#1b3a6b]">Açıklamalı analizi başlat <ArrowRight className="h-3.5 w-3.5" /></button></article>
      </section>

      <p className="px-2 text-center text-xs leading-5 text-slate-500">KazKaz AI karar desteği sağlar; muhasebe kaydı, bağımsız denetim görüşü veya yatırım tavsiyesi değildir. DSO hesabı için dönem gün sayısı; cari oran için dönen varlıklar zorunludur. Borç/özkaynak görünümü: {formatMultiple(debtToEquity)}. Brüt marj: {formatPercent(grossMargin)}. Tahsilat süresi: {dsoDays == null ? 'dönem gün sayısı gerekli' : `${dsoDays.toLocaleString('tr-TR', { maximumFractionDigits: 1 })} gün`}.</p>
    </div>
  );
};
