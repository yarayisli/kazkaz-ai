...
import {
  Activity, AlertTriangle, Bot, Building2, CheckCircle2, Clock3, CreditCard, Database,
...
const Kpi = ({ icon: Icon, label, value, detail, tone = 'violet' }: { icon: React.ElementType; label: string; value: string | number; detail: string; tone?: 'violet' | 'emerald' | 'amber' | 'sky' }) => {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.13em] text-slate-400">{label}</p><p className="mt-2 text-2xl font-black text-[#0f2252]">{value}</p></div><span className={`grid h-10 w-10 place-items-center rounded-xl ${tones[tone]}`}><Icon className="h-5 w-5" /></span></div><p className="mt-3 text-xs leading-5 text-slate-500">{detail}</p></article>;
};
...
export const PlatformAdminTab: React.FC = () => {
  ...
  // Şirket toplamı gerçek; aktif/pilot/üye kalemleri ilk N şirketlik örneklem
  // üzerinden hesaplanır. Örneklem tüm şirketleri kapsamıyorsa bunu belirt.
  const sayac = overview.sayaclar;
  const orneklemli = sayac.kapsam === 'orneklem';
  const orneklemAdedi = sayac.orneklem_sirket ?? sayac.orneklem_siniri ?? 0;
  const sirketDetay = orneklemli
    ? `${sayac.aktif_sirket} aktif · ${sayac.pilot_sirket} pilot · ilk ${orneklemAdedi} şirket örneklemi`
    : `${sayac.aktif_sirket} aktif · ${sayac.pilot_sirket} pilot`;
  const uyeDetay = orneklemli
    ? `İlk ${orneklemAdedi} şirketin üye toplamı (örneklem)`
    : 'Şirket üyeliklerinin toplamı';

  return <div className="space-y-5">
    <header className="overflow-hidden rounded-2xl bg-[#0f2252] p-6 text-white shadow-[0_18px_45px_rgba(15,34,82,.18)]">
      <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between"><div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-white/10"><ServerCog className="h-6 w-6" /></span><div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-violet-200">KazKaz işletim merkezi</p><h1 className="mt-1 text-2xl font-black">Platform yönetimi</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Şirketleri, AI sürekliliğini, operasyon kapılarını ve destek sinyallerini tek yerde yönetin.</p></div></div><button onClick={() => void load()} disabled={loading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 text-xs font-extrabold hover:bg-white/15 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Yenile</button></div>
    ...
    {section === 'overview' && <>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Kpi icon={Building2} label="Toplam şirket" value={sayac.toplam_sirket} detail={sirketDetay} /><Kpi icon={Users} label={orneklemli ? 'Üye (örneklem)' : 'Toplam üye'} value={sayac.toplam_uye} detail={uyeDetay} tone="sky" /><Kpi icon={Activity} label="Teknik kapı" value={`${readyChecks}/${totalChecks}`} detail={overview.canli_hazirlik.durum === 'hazir' ? 'Canlı kapısı hazır' : 'Eksik kontroller var'} tone={overview.canli_hazirlik.durum === 'hazir' ? 'emerald' : 'amber'} /><Kpi icon={Bot} label="AI sürekliliği" value={`${overview.ai.saglayicilar.filter(item => item.hazir).length}/${overview.ai.saglayicilar.length}`} detail="Hazır sağlayıcı sayısı" tone="violet" /><Kpi icon={AlertTriangle} label="Yeni bildirim" value={overview.sayaclar.yeni_geri_bildirim} detail="Mesaj içeriği gizli tutulur" tone="amber" /></div>
      <section className="panel-card p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.14em] text-violet-600">Dört haftalık müşteri kanıtı</p><h2 className="mt-1 font-black text-slate-900">Pilot ölçüm panosu</h2></div><span className={`rounded-full px-3 py-1 text-xs font-extrabold ${overview.pilot.asgari_kanit_hazir ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{overview.pilot.asgari_kanit_hazir ? 'Asgari kanıt hazır' : 'Kanıt birikiyor'}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Kpi icon={Building2} label="Pilot şirket" value={`${overview.pilot.pilot_sirket}/${overview.pilot.hedef_sirket_araligi}`} detail={`${overview.pilot.dort_haftayi_tamamlayan} şirket 4 haftayı tamamladı`} tone={overview.pilot.kapsam_gecerli ? 'emerald' : 'amber'} /><Kpi icon={CheckCircle2} label="Görev tamamlama" value={overview.pilot.gorev_tamamlama.oran_yuzde == null ? '—' : `%${overview.pilot.gorev_tamamlama.oran_yuzde}`} detail={`${overview.pilot.gorev_tamamlama.tam_yolculuk_sirket} şirket tüm akışı tamamladı`} tone="sky" /><Kpi icon={Clock3 as React.ElementType} label="Medyan çözüm" value={overview.pilot.destek.medyan_cozum_dakika == null ? '—' : `${overview.pilot.destek.medyan_cozum_dakika} dk`} detail={`${overview.pilot.destek.cozulen_sure_ornegi} çözülmüş talep örneği`} tone="violet" /><Kpi icon={AlertTriangle} label="Bildirilen hata" value={overview.pilot.bildirilen_hata.adet} detail="Müşteri hata talepleri; teknik 5xx değildir" tone="amber" /><Kpi icon={Users} label="Ücretli devam" value={overview.pilot.devam_niyeti.ucretli_devam_yuzde == null ? '—' : `%${overview.pilot.devam_niyeti.ucretli_devam_yuzde}`} detail={`${overview.pilot.devam_niyeti.ucretli_devam_yanit} müşteri yanıtı`} tone="emerald" /></div></section>
      <div className="grid gap-5 lg:grid-cols-2"><section className="panel-card p-5"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-violet-700" /><div><h2 className="font-black text-slate-900">Canlıya çıkış kapıları</h2><p className="mt-1 text-xs text-slate-500">Kritik ve operasyonel hazırlık</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{Object.entries(allChecks).map(([key,ok]) => <div key={key} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs font-bold text-slate-700"><StatusDot ok={ok} />{labelMap[key] || key}</div>)}</div></section><section className="panel-card p-5"><div className="flex items-center gap-3"><Bot className="h-5 w-5 text-violet-700" /><div><h2 className="font-black text-slate-900">AI sağlayıcı zinciri</h2><p className="mt-1 text-xs text-slate-500">Finans motoru her zaman doğruluk kaynağıdır</p></div></div><div className="mt-4 space-y-2">{overview.ai.saglayicilar.map(provider => <div key={provider.ad} className="flex items-center justify-between rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2"><StatusDot ok={provider.hazir} /><span className="text-sm font-extrabold capitalize text-slate-800">{provider.ad}</span></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold uppercase text-slate-500">{provider.rol}</span></div>)}</div><div className="mt-4 rounded-xl bg-violet-50 p-3 text-xs leading-5 text-violet-900">{overview.ai.politika}</div></section></div>
    </>}

    {section === 'companies' && <PlatformCompaniesPanel />}

    {section === 'operations' && <div className="grid gap-5 lg:grid-cols-2"><section className="panel-card p-5"><div className="flex items-center gap-3"><Activity className="h-5 w-5 text-violet-700" /><h2 className="font-black text-slate-900">Performans ölçümleri</h2></div><div className="mt-4 grid grid-cols-3 gap-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-bold uppercase text-slate-400">Örneklem</p><p className="mt-2 text-lg font-black">{overview.performans.genel?.orneklem ?? 0}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-bold uppercase text-slate-400">p50</p><p className="mt-2 text-lg font-black">{overview.performans.genel?.p50_ms ?? 0} ms</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-bold uppercase text-slate-400">p95</p><p className="mt-2 text-lg font-black">{overview.performans.genel?.p95_ms ?? 0} ms</p></div></div><div className="mt-4 space-y-2">{Object.entries(overview.performans.operasyonlar || {}).map(([name,item]) => <div key={name} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 text-xs"><span className="font-bold text-slate-700">{name.replaceAll('_',' ')}</span><span className="text-slate-500">{item.orneklem} çağrı · %{item.basari_orani} · p95 {item.p95_ms} ms</span></div>)}</div></section><div className="space-y-5"><section className="panel-card p-5"><div className="flex items-center gap-3"><CreditCard className="h-5 w-5 text-violet-700" /><h2 className="font-black text-slate-900">Ödeme hazırlığı</h2></div><p className="mt-4 text-sm font-extrabold text-slate-800">{overview.odeme.durum === 'hazir' ? 'Hazır' : 'Kurulum eksik'}</p><div className="mt-3 flex flex-wrap gap-2">{overview.odeme.eksikler?.map(item => <span key={item} className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">{labelMap[item] || item.replaceAll('_',' ')}</span>)}</div></section><section className="panel-card p-5"><div className="flex items-center gap-3"><Database className="h-5 w-5 text-violet-700" /><h2 className="font-black text-slate-900">ERP bağlantıları</h2></div><div className="mt-4 space-y-2">{Object.entries(overview.erp.saglayicilar).map(([name,item]) => <div key={name} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 text-xs"><span className="font-extrabold capitalize text-slate-800">{name}</span><span className="font-bold text-slate-500">{item.durum}</span></div>)}</div></section></div></div>}

    {section === 'events' && <section className="panel-card p-5"><div className="flex items-center gap-3"><Activity className="h-5 w-5 text-violet-700" /><div><h2 className="font-black text-slate-900">Destek ve denetim olayları</h2><p className="mt-1 text-xs text-slate-500">Mesaj gövdesi ve finansal değerler bu görünümde tutulmaz.</p></div></div><div className="mt-4 space-y-2">{events?.olaylar.map(event => <article key={`${event.tur}-${event.olay_id}`} className="flex flex-col gap-2 rounded-xl border border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><span className={`rounded-full px-2 py-1 text-xs font-black uppercase ${event.tur === 'geri_bildirim' ? 'bg-amber-50 text-amber-700' : 'bg-sky-50 text-sky-700'}`}>{event.tur}</span><p className="text-xs font-extrabold text-slate-800">{event.etiket}</p></div><p className="mt-1 text-xs text-slate-500">{event.sirket_adi}{event.sayfa ? ` · ${event.sayfa}` : ''}</p></div><div className="text-left sm:text-right"><p className="text-xs font-bold uppercase text-slate-500">{event.durum}</p><p className="mt-1 text-xs text-slate-400">{event.zaman?.slice(0,19).replace('T',' ') || '—'}</p></div></article>)}{!events?.olaylar.length && <p className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Henüz destek veya denetim olayı yok.</p>}</div></section>}
  </div>;
};
