...
const HealthBadge = ({ value }: { value: string }) => {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-black uppercase tracking-wide ${styles[value] || styles.hareketsiz}`}>{healthLabels[value] || value}</span>;
};
  ...
  return <div className="space-y-4">
    ...
    <div className={`grid gap-4 ${selectedId ? 'xl:grid-cols-[minmax(0,1fr)_minmax(420px,0.8fr)]' : ''}`}>
      <section className="panel-card overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><p className="text-xs font-extrabold text-slate-700">{filtered.length} şirket</p><p className="text-xs text-slate-400">Finansal değer gösterilmez</p></div>
        {loading ? <div className="grid min-h-64 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-violet-600" /></div> : <div className="divide-y divide-slate-100">
          {filtered.map(company => <button key={company.sirket_id} onClick={() => setSelectedId(company.sirket_id)} className={`grid w-full gap-3 p-4 text-left transition hover:bg-slate-50 sm:grid-cols-[minmax(190px,1.3fr)_110px_110px_150px_28px] sm:items-center ${selectedId === company.sirket_id ? 'bg-violet-50/60' : ''}`}>
            <div><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-black text-slate-900">{company.sirket_adi}</p><HealthBadge value={company.operasyon_sagligi} /></div><p className="mt-1 text-xs text-slate-500">{company.sektor} · {company.uye_sayisi} üye · {company.bekleyen_davet} davet</p></div>
            <div><p className="text-xs font-bold uppercase text-slate-400">Durum</p><p className="mt-1 text-xs font-extrabold text-slate-700">{statusLabels[company.durum] || company.durum}</p></div>
            <div><p className="text-xs font-bold uppercase text-slate-400">Paket</p><p className="mt-1 text-xs font-extrabold uppercase text-violet-700">{company.plan}</p></div>
            <div><p className="text-xs font-bold uppercase text-slate-400">Son hareket</p><p className="mt-1 truncate text-xs font-semibold text-slate-600">{formatDate(company.son_aktivite)}</p><p className="mt-0.5 truncate text-xs text-slate-400">{actionLabels[company.son_aksiyon] || company.son_aksiyon.replaceAll('_', ' ')}</p></div>
      ...
      {selectedId && <aside className="panel-card overflow-hidden xl:sticky xl:top-20 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-5 py-4"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">Şirket ayrıntısı</p><p className="mt-1 text-sm font-black text-slate-900">{detail?.sirket.sirket_adi || 'Yükleniyor'}</p></div><button aria-label="Şirket ayrıntısını kapat" onClick={() => setSelectedId(null)} className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200"><X className="h-4 w-4" /></button></div>
        {detailLoading && !detail ? <div className="grid min-h-72 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-violet-600" /></div> : detail && <div className="space-y-5 p-5">
            ...
            <div role="status" aria-live="polite" className={`rounded-xl border p-3 text-xs font-bold leading-5 ${bekleyenIptal > 0 ? 'border-amber-300 bg-amber-50 text-amber-900' : 'border-sky-200 bg-sky-50 text-sky-800'}`}>
              ...
              {bekleyenIptal > 0 && (
                <button
                  ...
                  className="mt-2 inline-flex items-center gap-1.5 rounded-lg border border-amber-400 bg-white px-3 py-1.5 text-xs font-bold text-amber-800 hover:bg-amber-100 disabled:opacity-50"
                >
          ...
          <section><div className="flex items-center gap-2"><Activity className="h-4 w-4 text-violet-600" /><h3 className="text-xs font-black text-slate-900">Şirket ne yapıyor?</h3></div><div className="mt-3 space-y-2">{detail.son_olaylar.slice(0, 8).map((event, index) => <div key={`${event.aktor}-${event.zaman}-${index}`} className="flex gap-3 rounded-xl border border-slate-200 p-3"><span className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-violet-50"><FileClock className="h-3.5 w-3.5 text-violet-700" /></span><div className="min-w-0"><p className="truncate text-xs font-extrabold text-slate-700">{actionLabels[event.aksiyon] || event.aksiyon.replaceAll('_', ' ')}</p><p className="mt-1 text-xs text-slate-400">{event.aktor_rolu} · {event.aktor} · {formatDate(event.zaman)}</p></div></div>)}{!detail.son_olaylar.length && <p className="rounded-xl border border-dashed border-slate-300 p-4 text-center text-xs text-slate-400">Henüz denetlenebilir aktivite yok.</p>}</div></section>

          <section><div className="flex items-center gap-2"><Users className="h-4 w-4 text-violet-600" /><h3 className="text-xs font-black text-slate-900">Üyeler ve roller</h3></div><div className="mt-3 space-y-2">{detail.uyeler.map(member => <div key={member.kullanici_ozeti} className="flex items-center justify-between rounded-xl bg-slate-50 p-3"><div className="flex items-center gap-2"><UserRound className="h-4 w-4 text-slate-400" /><div><p className="text-xs font-extrabold text-slate-700">{member.eposta_maskeli}</p><p className="text-xs text-slate-400">{member.kullanici_ozeti}</p></div></div><span className="rounded-full bg-white px-2 py-1 text-xs font-black uppercase text-violet-700">{member.rol}</span></div>)}</div></section>

          <section><div className="flex items-center gap-2"><Mail className="h-4 w-4 text-violet-600" /><h3 className="text-xs font-black text-slate-900">Destek sinyalleri</h3></div><div className="mt-3 space-y-2">{detail.geri_bildirimler.slice(0, 8).map(item => <div key={item.geri_bildirim_id} className="rounded-xl border border-slate-200 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold text-slate-700">{item.talep_no ? `${item.talep_no} · ` : ''}{item.kategori} · {item.sayfa}</p><p className="mt-1 text-xs text-slate-400">{formatDate(item.zaman)}{item.iletisim_izni ? ' · iletişim izni var' : ''}{item.yanit_verildi ? ' · yanıtlandı' : ''}{item.durum === 'resolved' ? (item.memnun == null ? ' · memnuniyet bekleniyor' : item.memnun ? ' · 👍 memnun' : ' · 👎 memnun değil') : ''}</p></div><select aria-label={`${item.kategori} geri bildirim durumu`} value={item.durum} disabled={actionLoading} onChange={event => void updateFeedback(item.geri_bildirim_id, event.target.value as FeedbackStatus)} className="min-h-8 rounded-lg border border-slate-300 bg-white px-2 text-xs font-bold"><option value="new">Yeni</option><option value="in_review">İnceleniyor</option><option value="resolved">Çözüldü</option></select></div></div>)}{!detail.geri_bildirimler.length && <p className="rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-700"><CheckCircle2 className="mr-2 inline h-4 w-4" />Açık destek sinyali yok.</p>}</div></section>

          <section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4"><div className="flex items-center gap-2"><ShieldAlert className="h-4 w-4 text-amber-700" /><h3 className="text-xs font-black text-amber-900">Kontrollü müdahale</h3></div><p className="mt-2 text-xs leading-5 text-amber-800">Her işlem denetim kaydına yazılır. Finansal veri görüntülenmez veya değiştirilmez.</p><textarea value={reason} onChange={event => setReason(event.target.value)} maxLength={300} placeholder="Müdahale gerekçesi (oturum kapatma için zorunlu)" className="mt-3 min-h-20 w-full resize-none rounded-xl border border-amber-200 bg-white p-3 text-xs outline-none focus:border-amber-400" />
            <div className="mt-3 grid grid-cols-2 gap-2"><select aria-label="Şirket durumunu değiştir" value={detail.sirket.durum} disabled={actionLoading} onChange={event => void updateCompany({ durum: event.target.value })} className="min-h-10 rounded-xl border border-slate-300 bg-white px-3 text-xs font-bold"><option value="active">Aktif</option><option value="pilot">Pilot</option><option value="suspended">Askıya al</option><option value="closed">Kapat</option></select><select aria-label="Şirket paketini değiştir" value={detail.sirket.plan} disabled={actionLoading} onChange={event => void updateCompany({ plan: event.target.value })} className="min-h-10 rounded-xl border border-slate-300 bg-white px-3 text-xs font-bold uppercase"><option value="free">Free</option><option value="trial">Trial</option><option value="pro">Pro</option><option value="uzman">Uzman</option></select></div>
            <button onClick={() => void revokeSessions()} disabled={actionLoading || reason.trim().length < 5} className="mt-2 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-red-600 px-3 text-xs font-black text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-40">{actionLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />} Tüm oturumları sonlandır</button>
          </section>

          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs leading-5 text-emerald-800"><ShieldCheck className="mr-2 inline h-4 w-4" />E-postalar maskeli, kullanıcı kimlikleri özetlidir. Müşteri dosyası, finansal rakam ve geri bildirim mesajı açılmaz.</div>
        </div>}
      </aside>}
    </div>
  </div>;
