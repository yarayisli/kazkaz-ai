...
export const ReportsTab: React.FC<ReportsTabProps> = ({ data, onNavigateDataEntry }) => {
  ...
  const redownload = async (report: ArsivRaporu, type: 'pdf' | 'excel') => {
    ...
    try {
      const sonuc = await arsivRaporuIndir(report.rapor_id, type);
      setMessage(
        sonuc.ozgunCikti
          ? `${report.donem} · ${report.surum} için üretim anında saklanan, bütünlüğü doğrulanmış özgün rapor indirildi.`
          : sonuc.yenidenUretildi
          ? `${report.donem} · ${report.surum} raporu indirildi. Not: bu rapor saklı verinizden güncel motorla (${sonuc.motorGuncel ?? '?'}) yeniden üretildi; arşiv sürümü (${sonuc.motorArsiv ?? '?'}) farklı olduğundan biçim/kalemler özgün rapordan sapabilir.`
          : `${report.donem} · ${report.surum} eski arşiv kaydından yeniden oluşturularak indirildi.`,
      );
    }
    ...
    <div className="space-y-5 pb-8">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="panel-card p-5 lg:col-span-2">
          ...
          <p className="mt-3 text-xs leading-5 text-slate-600">Her yeni raporun özgün PDF/Excel çıktısı bütünlük özetiyle saklanır. İki sürüm seçerek temel finansal farkları karşılaştırabilirsiniz.</p>
          {!canDownload ? <div className="mt-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-xs text-slate-500">Arşiv için doğrulanmış şirket oturumu gerekir.</div> : archive.length ? <div className="mt-4 space-y-2">{archive.map((report) => <article key={report.rapor_id} className={`flex flex-col gap-3 rounded-xl border p-3 sm:flex-row sm:items-center sm:justify-between ${selectedReports.includes(report.rapor_id) ? 'border-violet-300 bg-violet-50' : 'border-slate-200 bg-slate-50'}`}><label className="flex min-w-0 cursor-pointer items-start gap-3"><input type="checkbox" checked={selectedReports.includes(report.rapor_id)} onChange={() => toggleCompare(report.rapor_id)} className="mt-1 accent-violet-700" /><span className="min-w-0"><span className="block truncate text-xs font-extrabold text-slate-900">{report.donem} · {report.surum}</span><span className={`mt-1 inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-bold ${report.ozgun_cikti ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'border-amber-300 bg-amber-50 text-amber-700'}`} title={report.ozgun_cikti ? 'Üretim anındaki dosya bütünlük özetiyle saklanır.' : 'Eski arşiv kaydı; indirmede saklı finansal veriden yeniden oluşturulur.'}>{report.ozgun_cikti ? 'Özgün çıktı' : 'Eski arşiv kaydı'}</span>{report.guncel_motor === false && !report.ozgun_cikti && <span className="ml-1 mt-1 inline-flex items-center rounded-full border border-amber-300 bg-amber-50 px-2 py-0.5 text-xs font-bold text-amber-700" title={`Arşiv motoru ${report.motor_surumu ?? '?'}; indirmede güncel motorla yeniden üretilir.`}>Farklı motor sürümü</span>}<span className="mt-1 block text-xs text-slate-500">Ciro {money(report.ozet.revenue)} · Net kâr {money(report.ozet.netProfit)}</span></span></label><div className="flex flex-wrap items-center gap-2">{report.formatlar.map((type) => <button type="button" key={type} disabled={loading !== null} onClick={() => void redownload(report, type)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-extrabold uppercase text-slate-700 hover:border-violet-300">{type} indir</button>)}{canDelete && <button type="button" aria-label={`${report.donem} raporunu sil`} disabled={archiveLoading} onClick={() => void removeReport(report)} className="grid h-8 w-8 place-items-center rounded-lg border border-red-200 bg-white text-red-700 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /></button>}</div></article>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-xs text-slate-500">Henüz arşivlenmiş rapor yok. İlk PDF veya Excel raporunu ürettiğinizde burada görünecek.</div>}
          {comparison.length === 2 && <div className="mt-5 rounded-xl border border-violet-200 bg-violet-50 p-4"><div className="flex items-center gap-2 text-xs font-extrabold text-violet-900"><GitCompare className="h-4 w-4" /> Seçili sürüm karşılaştırması</div><div className="mt-3 grid grid-cols-3 gap-2 text-xs"><span className="font-bold text-slate-500">Metrik</span><span className="font-bold text-slate-700">{comparison[0].donem}</span><span className="font-bold text-slate-700">{comparison[1].donem}</span>{[['Ciro','revenue'],['Net kâr','netProfit'],['Nakit','cash'],['Toplam borç','totalDebt']].map(([label,key]) => <React.Fragment key={key}><span className="text-slate-500">{label}</span><span className="font-bold text-slate-900">{money(comparison[0].ozet[key as keyof ArsivRaporu['ozet']] as number)}</span><span className="font-bold text-slate-900">{money(comparison[1].ozet[key as keyof ArsivRaporu['ozet']] as number)}</span></React.Fragment>)}</div></div>}
        </article>
        <article className="panel-card p-5">
