...
export const SourceLockPanel: React.FC<SourceLockPanelProps> = ({ dogrulama }) => {
  ...
  return (
    <section className="panel-card p-4">
      ...
      <ul className="space-y-1.5">
        {eslesmeler.map((eslesme, index) => (
          ...
          >
            ...
            <span className="shrink-0 text-xs font-extrabold tabular-nums text-[#0f1729]">{eslesme.ham}</span>
            <span className="min-w-0 flex-1 truncate text-right text-xs text-slate-500">{eslesme.kaynak}</span>
          </li>
        ))}
        ...
        {reddedilen.map((ham, index) => (
          ...
          >
            ...
            <span className="shrink-0 text-xs font-extrabold tabular-nums text-red-700">{ham}</span>
            <span className="min-w-0 flex-1 truncate text-right text-xs font-semibold text-red-600">
              Kaynak bulunamadı
            </span>
          </li>
        ))}
      ...
      <p className="mt-3 flex items-start gap-2 border-t border-slate-100 pt-3 text-xs leading-4 text-slate-500">
