    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="panel-card p-5">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Aktif şirket</p><h2 className="text-base font-extrabold text-slate-900">{userProfile?.companyName || companyName}</h2></div>
          </div>
        ...
        <section className="panel-card p-5">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-700"><Users className="h-5 w-5" /></span><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Erişim</p><h2 className="text-base font-extrabold text-slate-900">Rol tabanlı yetkilendirme</h2></div></div>
          ...
          <div className="mt-4 flex items-start gap-2 rounded-xl bg-emerald-50 p-3 text-xs leading-5 text-emerald-800"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0" /> Şirket üyeliği ve rol değişiklikleri yalnız doğrulanmış backend üzerinden uygulanır; istemci kendi rolünü değiştiremez.</div>
        </section>
      </div>
      ...
      <section className="panel-card p-5 sm:p-6">
        ...
        {!canManageMembers ? <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">Kullanıcı daveti ve rol değişikliği yalnız şirket Admin rolüne açıktır.</p> : <>
          ...
          {members?.davetler.length ? <div className="mt-5"><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-400">Bekleyen davetler</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{members.davetler.map((invite) => <div key={invite.davet_id} className="rounded-xl border border-sky-100 bg-sky-50 p-3"><p className="truncate text-xs font-bold text-sky-950">{invite.eposta}</p><p className="mt-1 text-xs text-sky-700">{invite.rol} · 7 gün içinde kabul</p></div>)}</div></div> : null}
        </>}
      </section>
      ...
      <section className="panel-card p-5 sm:p-6">
        ...
        <p className="mt-4 text-xs leading-4 text-slate-500">Silme yalnızca Admin ve CFO rollerine açıktır. Yedeklerdeki veriler tanımlanmış saklama ve imha politikasına göre ayrıca temizlenmelidir.</p>
      </section>
    </div>
  );
};
